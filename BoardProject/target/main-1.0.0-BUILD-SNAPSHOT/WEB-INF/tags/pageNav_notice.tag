...
<%@ attribute name="pageObject" required="true"
 type="com.webjjang.board.PageObject.pageObject_notice" %>
	...
	<c:forEach begin="${pageObject.startPage }" end="${pageObject.endPage }" var="cnt">
  	<li ${(pageObject.page == cnt)?"class=\"active\"":"" } 
  	 data-page=${cnt }>
  	 	...
  	 	<c:if test="${pageObject.page != cnt }">
  			<a href="${listURI }?page=${cnt }&perPageNum=${pageObject.perPageNum}${query}&word=${pageObject.word}&start_date=${pageObject.start_date}&end_date=${pageObject.end_date}"
