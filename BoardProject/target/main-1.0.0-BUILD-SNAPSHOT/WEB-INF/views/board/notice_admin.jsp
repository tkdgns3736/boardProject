...
.jb-xx-large { font-size: xx-large; }
 ...
 <style>
	...
	function popup() {
		...
		//날짜설정
		if( date_check.end_date.value == "" ) {
			date_check.end_date.focus();
			alert("팝업 유지 날짜를 정해주세요."); 
			return false; 
			}
		...
		if (r == true) {
			...
			$.ajax({
				type : "POST",
				url : "popup_push.do",
				data : {"value":chk_arr.toString(),"end_date":$("#end_date").val()},
				success : function(data) {
					alert("등록 성공")
					window.location.reload(true);
				},
	...
	<div class="container" style="background-color: #EAEAEA;">
		...
		<nav>
			...
			<div style="border: 1px solid #EAEAEA; padding: 10px; height: auto; min-height: 30px; overflow: auto ;">
				<h2 style="display:inline;"><a href="notice_admin.do">공지사항 관리</a></h2><p style="display:inline;">&nbsp;&nbsp;&nbsp;공지사항 관리 페이지.</p>
		...
		<div style="text-align: right;">
			<form class="navbar-form" id="date_check" name="date_check">

				<div style="display: inline">
					<input type="date" name="start_date"
						value="${pageObject.start_date}">~<input type="date"
						name="end_date" id="end_date" value="${pageObject.end_date}">
				</div>
				<div style="display: inline">
			...
			<tr class="jb-small">
				...
				<tr>
					<td colspan="10" style="text-align: center"><pageObject:pageNav_notice
							listURI="notice_admin.do" pageObject="${pageObject }" /></td>	
				</tr>
			</table>
					<input  style='float: reft; display:inline;" 'type='button'  onclick="location.href='notice_write.do'" value='새 공지사항' />
			<button style="float: right; "type="button" onclick="notice_delete();">선택 항목 삭제</button>
			<button style="float: right; "type="button" onclick="popup();">팝업 등록</button>
